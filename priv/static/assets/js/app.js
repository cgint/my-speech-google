...
`
  ...
  var TtsStreamPlayer = class {
    ...
    ensureContext() {
      if (!this.audioCtx) {
        this.audioCtx = new AudioContext({ sampleRate: 24e3 });
    ...
    enqueueChunk(sessionId, pcmB64, sampleRate) {
      ...
      const audioBuffer = this.audioCtx.createBuffer(1, pcm.length, sampleRate || 24e3);
  ...
  var ttsPlayer = new TtsStreamPlayer();
